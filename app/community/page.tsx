...
useEffect(() => {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      const snap = await getDocs(collectionGroup(db, "watched"));
...
function ActivityItem({ activity }: { activity: any }) {
  ...
  if (activity.type === "watched") {
    ...
    text = `${activity.username} watched ${activity.movieTitle}`;
  }

  if (activity.type === "rated") {
    color = "text-yellow-400";
    text = `${activity.username} rated ${activity.movieTitle} ⭐ ${activity.rating}/5`;
  }

  if (activity.type === "reviewed") {
    color = "text-blue-400";
    text = `${activity.username} reviewed ${activity.movieTitle}`;
  }

  if (activity.type === "watchlist") {
    color = "text-purple-400";
    text = `${activity.username} added ${activity.movieTitle} to Watchlist`;
  }
...
return (
  <div className="p-10 flex gap-10">
    ...
    {/* Activity */}
    <div className="w-2/3">
      <h2 className="text-3xl font-bold mb-8">📊 Activity Feed</h2>
